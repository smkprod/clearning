...
type TaskExtra = Pick<Task, 'realWorld' | 'starter' | 'doneWhen' | 'sandbox'>;
...
const algo: TaskSeed[] = [
  t('algo-w1', 'algo', 'practice', 'Секундомер Stopwatch: твой первый замер',
    ...
    {
      realWorld: 'Так профилируют медленные места: «почему этот endpoint отвечает 2 секунды?». Оборачиваешь подозрительный код в Stopwatch и находишь виновника. В твоём Auslagenerstattung можно замерить, сколько занимает выборка расходов из БД. Взрослая версия — BenchmarkDotNet.',
...
Console.WriteLine($"sum = {sum}, заняло {sw.ElapsedMilliseconds} мс");`,
  ...
  t('algo-w2', 'algo', 'practice', 'Big-O на пальцах: три цикла',
    ...
    {
      realWorld: 'Пока данных мало (10 записей) — всё летает и разницы не видно. Но когда придут 100 000 пользователей, случайный O(n²) в коде кладёт сервер. Половина реальных инцидентов «сайт тормозит/лёг» — это вложенный цикл или запрос в цикле, который никто не заметил на тестовых данных.',
      sandbox: 'dotnetfiddle',
...
Console.WriteLine($"n = {data.Length}, O(n^2) занял {sw.ElapsedMilliseconds} мс");`,
  ...
  t('algo-01', 'algo', 'practice', 'List vs HashSet: почувствуй разницу',
    ...
    {
      realWorld: 'Классика: «этот email уже зарегистрирован?». Проверять по списку из 100k юзеров через List.Contains — запрос висит; HashSet отвечает мгновенно. Дедупликация, «уже обработанные id», быстрые проверки принадлежности — везде HashSet. В Telegram Mini App так проверяют, не нажал ли юзер кнопку дважды.',
      sandbox: 'dotnetfiddle',
...
Console.WriteLine($"HashSet: {sw.ElapsedMilliseconds} мс");`,
  ...
  t('algo-02', 'algo', 'practice', 'Реши Two Sum через словарь',
    ...
    { realWorld: 'Хеш-подход «положил в словарь → мгновенно нашёл» — самый частый способ убрать вложенный цикл в реальном коде. Сопоставить платежи с заказами, найти дубли, сгруппировать записи. Это же и топ-1 тип задач на технических собеседованиях.' }),
  t('algo-03', 'algo', 'practice', 'Группировка анаграмм',
    ...
    undefined,
    { realWorld: 'Группировка по вычисленному ключу — ежедневная операция: заказы по клиенту, логи по дню, расходы по категории. LINQ GroupBy делает ровно это. Приём «нормализуй элемент → используй как ключ словаря» переносится прямо в рабочий код.' }),
  t('algo-04', 'algo', 'practice', 'Подсчёт частот символов',
    ...
    undefined,
    { realWorld: 'Подсчёт частот — основа многих фич: топ-N популярных товаров, сколько раз встречается тег, аналитика «какие категории расходов чаще всего». Dictionary<ключ, счётчик> — рабочая лошадка.' }),
  t('algo-05', 'algo', 'practice', 'Two pointers: два указателя',
    ...
    undefined,
    { realWorld: 'Два указателя — эффективная работа с отсортированными данными без лишних проходов: слияние двух списков, поиск пары, схлопывание соседних дублей. Частый приём на собесе и в обработке отсортированных выборок.' }),
  t('algo-06', 'algo', 'practice', 'Sliding window: скользящее окно',
    ...
    undefined,
    { realWorld: 'Скользящее окно — как считают метрики «за последние N»: скользящее среднее, rate limiting (сколько запросов за минуту), самый активный период. Рабочий приём для потоков данных.' }),
  t('algo-07', 'algo', 'practice', 'Рекурсия ↔ цикл',
    ...
    { realWorld: 'Рекурсия — это обход всего древовидного: файловая система, категории с подкатегориями, JSON, комментарии с ответами. Но глубокая рекурсия = StackOverflow на проде, поэтому важно уметь и переписать её в цикл.' }),
  t('algo-08', 'algo', 'theory', 'Сортировки: принцип, не зубрёжка',
    ...
    { realWorld: 'Сам сортировки писать не будешь (есть List.Sort / OrderBy), но понимать обязан: почему это O(n log n), что такое устойчивость (стабильный порядок равных). Спросят и на собесе, и на AP2.' }),
  t('algo-09', 'algo', 'practice', 'Бинарный поиск руками',
    ...
    { realWorld: 'Идея «дели пополам» повсюду: поиск по индексу в БД, git bisect для поиска коммита с багом, автодополнение. Классический вопрос на собесе — и легко ошибиться в границах, поэтому пишут руками.' }),
  t('algo-10', 'algo', 'practice', 'Struktogramm → код и обратно',
    ...
    { realWorld: 'Прямая тема письменного AP2: перевести алгоритм в Struktogramm/псевдокод и обратно. В работе — тот же навык «объяснить логику до кода», когда рисуешь решение на доске с командой.' }),
  t('algo-11', 'algo', 'practice', 'C#-метод → Pseudocode формата IHK',
    ...
    undefined,
    { realWorld: 'Формат твоего слабейшего экзаменационного блока (Algorithmen). А навык «читать чужой код и излагать его логику словами» нужен на каждом code review.' }),
  t('algo-12', 'algo', 'practice', 'Stack и Queue руками',
    ...
    undefined,
    { realWorld: 'Стек — это Undo/Redo, кнопка «назад» в браузере, стек вызовов. Очередь — обработка задач по порядку, сообщения, печать. В твоём Telegram-боте входящие апдейты обрабатываются как очередь.' }),
  t('algo-13', 'algo', 'practice', 'Строки: StringBuilder против конкатенации',
    ...
    undefined,
    { realWorld: 'Классическая утечка производительности: собирать большой текст/HTML/CSV-отчёт через += в цикле. На 100k строк это заметно тормозит из-за immutability строк. StringBuilder — стандартное решение и частый вопрос на собесе.' }),
  t('algo-14', 'algo', 'practice', 'Двумерные массивы: матрицы',
    ...
    undefined,
    { realWorld: 'Матрицы — это таблицы, игровые поля, картинки (пиксели), матрицы смежности графов. И регулярная тема трассировки на письменном AP2.' }),
  t('algo-15', 'algo', 'practice', 'Связный список: пойми ссылки',
    ...
    undefined,
    { realWorld: 'Сам редко пишешь, но понимать обязан: это основа устройства LinkedList и классический вопрос на собесе (reverse, поиск цикла). Учит думать про ссылки — а ссылки в C# везде.' }),
  t('algo-16', 'algo', 'practice', 'Множества в задачах',
    ...
    undefined,
    { realWorld: 'HashSet — рефлекс на «уникальность» и «принадлежность»: убрать дубли из выборки, «показывали ли уже это юзеру», пересечение прав доступа. Быстро и читаемо.' }),
  t('algo-17', 'algo', 'practice', 'Жадные алгоритмы базово',
    ...
    undefined,
    { realWorld: 'Жадные алгоритмы — планирование и распределение: сдача монетами, расписание без пересечений, выбор задач под дедлайн. Учит распознавать, когда «бери лучшее сейчас» даёт оптимум, а когда обманывает.' }),
  t('algo-18', 'algo', 'practice', 'Повторение: 5 задач заново по памяти',
    ...
    undefined,
    { realWorld: 'Активное вспоминание — самый эффективный способ закрепить. Решил через неделю заново без подсказок — навык твой и на собесе не подведёт. Не решил — вот где надо доработать.' }),
];
...
const csharp: TaskSeed[] = [
  t('cs-w1', 'csharp', 'practice', 'LINQ-разминка: Where / Select / OrderBy',
    ...
    {
      realWorld: 'LINQ — это буквально каждый экран с данными. В твоём Auslagenerstattung «показать расходы за месяц по категории» — это Where + OrderBy. А EF Core превращает точно такой же db.Expenses.Where(...) в SQL-запрос к базе. Освоил LINQ на списке — понял, как работает выборка данных во всём .NET.',
      sandbox: 'dotnetfiddle',
...
Console.WriteLine($"Старше 30: {over30.Count}");`,
  ...
  t('cs-01', 'csharp', 'practice', 'Докажи deferred execution в LINQ',
    ...
    { realWorld: 'Из-за этого в EF Core db.Users.Where(...) НЕ идёт в базу, пока ты не сделаешь .ToList(). Понимаешь deferred execution — понимаешь, почему один код бьёт в БД один раз, а другой — сто раз в цикле. Прямая причина тормозящих страниц.' }),
  t('cs-02', 'csharp', 'practice', 'Делегаты, Func/Action, события',
    ...
    { realWorld: 'Делегаты и события — это callbacks: «когда придёт сообщение — вызови этот метод», кнопки в UI, обработчики. Весь LINQ принимает делегаты (лямбды). В Telegram-боте «на команду /start выполни X» — это подписка на событие.' }),
  t('cs-03', 'csharp', 'practice', 'Свой generic-репозиторий',
    ...
    { realWorld: 'Generic-репозиторий — очень распространённый паттерн в .NET-проектах: один Repository<T> вместо копипасты CRUD под каждую сущность. List<T>, Dictionary<K,V>, DbSet<T> — все дженерики. Понимание where T: спрашивают на собесе.' }),
  t('cs-04', 'csharp', 'practice', 'async/await: воспроизведи дедлок и почини',
    ...
    { realWorld: 'Каждый запрос к БД, каждый вызов чужого API, каждое чтение файла в ASP.NET Core — async. Ошибка с .Result роняет прод под нагрузкой (thread pool starvation) — это реальный инцидент, который ищут часами. Первый вопрос про async почти на любом .NET-собесе.' }),
  t('cs-05', 'csharp', 'practice', 'Перепиши старый код на современный C#',
    ...
    { realWorld: 'Это то, как выглядит современный C#-код в 2026: records для DTO, pattern matching вместо лестниц if, nullable-аннотации ловят NullReferenceException ещё до запуска. Работодатель по этому сразу видит, писал ли ты код недавно или по старым туториалам.' }),
  t('cs-06', 'csharp', 'practice', 'Память: докажи boxing бенчмарком',
    ...
    { realWorld: 'value vs reference и boxing — это «почему тут копия, а там ссылка» и «почему этот код грузит GC». Реально всплывает в багах с изменением объектов и в оптимизации горячего кода. И это железобетонный вопрос собеса — многие джуны плавают.' }),
  t('cs-07', 'csharp', 'practice', 'Исключения vs Result-паттерн',
    ...
    { realWorld: 'Как обрабатывать ошибки — ежедневное архитектурное решение. «Юзер ввёл неверные данные» — это не исключение, а ожидаемый Result; «БД недоступна» — исключение. Правильный выбор делает код чище и API предсказуемее. Любят обсуждать на собесе senior-уровня.' }),
  t('cs-08', 'csharp', 'practice', 'LINQ-ката: 5 задач',
    ...
    [l('101 LINQ samples', 'https://learn.microsoft.com/samples/dotnet/try-samples/101-linq-samples/')],
    { realWorld: 'Именно эти операторы ты будешь писать каждый день: сгруппировать заказы по месяцу (GroupBy), развернуть вложенные списки (SelectMany), посчитать сумму (Aggregate), построить словарь-справочник (ToDictionary). Беглость в LINQ = скорость реальной работы с данными.' }),
  t('cs-09', 'csharp', 'practice', 'Интерфейс vs абстрактный класс',
    ...
    undefined,
    { realWorld: 'Интерфейсы — основа тестируемого кода (мокаешь IRepository) и всего DI в ASP.NET Core. «Интерфейс или абстрактный класс» — топ-вопрос немецких собесов и прямая тема ООП на письменном AP2.' }),
  t('cs-10', 'csharp', 'practice', 'List и Dictionary под капотом',
    ...
    undefined,
    { realWorld: 'Понимание, как List и Dictionary устроены внутри, объясняет их скорость и подсказывает, когда задать начальный Capacity в горячем коде. GetHashCode/Equals реально важны, когда кладёшь свои объекты в HashSet/Dictionary — иначе тихие баги. Частый вопрос собеса.' }),
  t('cs-11', 'csharp', 'practice', 'Файлы и JSON',
    ...
    { realWorld: 'JSON — язык общения всех API. Каждый ответ веб-API, каждая настройка в appsettings.json, каждое сообщение от Telegram — это JSON, который надо разобрать в объекты и обратно. В проекте P1 ты именно так хранишь расходы в файле.' }),
  t('cs-12', 'csharp', 'practice', 'HttpClient: сходи в чужой API',
    ...
    undefined,
    { realWorld: 'HttpClient — как твой сервис ходит в чужие сервисы: платёжка, карты, погода, Telegram Bot API. Обработка таймаутов и не-200 ответов — не мелочь: без неё падает прод, когда чужой API тормозит. Ровно это ты делаешь в Telegram Mini App.' }),
  t('cs-13', 'csharp', 'practice', 'Extension methods и yield return',
    ...
    undefined,
    { realWorld: 'Extension methods — как весь LINQ «прикручен» к коллекциям; в командах их пишут для читаемых хелперов. yield return даёт ленивую обработку — читать огромный файл построчно, не загружая целиком в память. Понимаешь yield — понимаешь, почему LINQ ленивый.' }),
  t('cs-14', 'csharp', 'practice', 'Самопроверка C#: 20 вопросов вслух',
    ...
    undefined,
    { realWorld: 'Это репетиция реального технического собеседования: там надо не просто знать, а внятно объяснить вслух под лёгким стрессом. Проговаривание вскрывает темы, которые «вроде понял», но связно сказать не можешь.' }),
];
...
const backend: TaskSeed[] = [
  t('be-w1', 'backend', 'practice', 'Подними пустой Web API и разбери Program.cs',
    ...
    { realWorld: 'Program.cs — точка входа КАЖДОГО ASP.NET Core приложения, включая твой Auslagenerstattung. Понимаешь эти 20 строк (DI, middleware, запуск) — понимаешь, как вообще устроен любой .NET-бэкенд. С этого начинается проект P2.' }),
  t('be-01', 'backend', 'practice', 'DI lifetimes: докажи экспериментом',
    ...
    { realWorld: 'Каждый сервис в твоём Auslagenerstattung внедряется через DI. Неправильный lifetime — реальный баг: DbContext как Singleton крашит приложение под нагрузкой, а Scoped-сервис в Singleton тихо отдаёт устаревшие данные. Первое, что спрашивают на собесе про ASP.NET Core.' }),
  t('be-02', 'backend', 'practice', 'Напиши свой middleware',
    ...
    { realWorld: 'Через middleware проходит каждый HTTP-запрос: аутентификация, логирование, CORS, обработка ошибок — всё это middleware. Порядок регистрации критичен (авторизация до эндпоинта, не после) — перепутал и получил дыру в безопасности. Свой middleware пишут в реальных проектах регулярно.' }),
  t('be-03', 'backend', 'practice', 'Воспроизведи и почини N+1',
    ...
    { realWorld: 'Задача №1 виновник тормозящих .NET-приложений: показываешь список заказов, а EF под капотом делает +1 запрос на каждую строку → 200 запросов вместо двух. На тестовых 5 записях незаметно, на проде — страница грузится 10 секунд. Умение это увидеть и починить ценят очень.' }),
  t('be-04', 'backend', 'practice', 'Change tracking и миграции руками',
    ...
    { realWorld: 'Миграции — как меняют схему БД в команде без ручного SQL и без потери данных на проде: добавил поле → миграция → накатилась на всех окружениях. Change tracking объясняет, почему SaveChanges сохраняет именно то, что ты менял. Ежедневная работа с EF Core.' }),
  t('be-05', 'backend', 'theory', 'REST-дизайн: чеклист',
    ...
    { realWorld: 'Правильные статус-коды и идемпотентность — то, за что фронтендеры и другие сервисы скажут спасибо: 404 когда нет, 400 когда невалидно, 201 при создании. Плохой REST-дизайн видно сразу и на code review, и на собесе. Твой API в проектах P2-P3 будут судить по этому.' }),
  t('be-06', 'backend', 'practice', 'JWT auth с нуля',
    ...
    { realWorld: 'JWT — как устроен логин почти в каждом современном приложении (включая твой Telegram Mini App и любой SPA + API). Пользователь входит → получает токен → шлёт его в заголовке каждого запроса. Обязательно спросят на собесе и обязательно понадобится в проекте P3.' }),
  t('be-07', 'backend', 'practice', 'Валидация + единая обработка ошибок',
    ...
    { realWorld: 'Валидация на входе — первая линия защиты API от мусора и атак. Единый обработчик ошибок = клиент всегда получает понятный ответ, а не голый стектрейс (который ещё и утечка данных). Это стандарт любого продакшн-API и часть проекта P3.' }),
  t('be-08', 'backend', 'project', 'Auslagenerstattung: слой контроллеров',
    ...
    undefined,
    { realWorld: 'Это твоя цель №2 в чистом виде: на собесе покажут твой же код и спросят «объясни». Уметь без AI рассказать каждый эндпоинт своего проекта — то, что отличает «сделал с AI» от «понимаю и могу поддерживать».' }),
  t('be-09', 'backend', 'project', 'Auslagenerstattung: сервисы и EF',
    ...
    undefined,
    { realWorld: 'Слой данных — где живут самые каверзные вопросы собеса (N+1, транзакции, ленивая загрузка). Разобрав его в своём проекте, ты отвечаешь на них не теорией, а «вот как у меня сделано и почему».' }),
  t('be-10', 'backend', 'practice', 'Конфигурация и Options pattern',
    ...
    { realWorld: 'Одно и то же приложение, разные настройки для локали, теста и прода (строки подключения, ключи) — без пересборки, через appsettings и переменные окружения. Секреты в git — реальная утечка, за которую увольняют. Понадобится при деплое в Azure (проект P5).' }),
  t('be-11', 'backend', 'practice', 'Структурное логирование с Serilog',
    ...
    { realWorld: 'Когда на проде что-то сломалось, логи — единственный способ понять что. Структурные логи ({UserId}, {OrderId}) позволяют искать и фильтровать, а не читать простыню текста. «Как ты дебажишь прод?» — реальный вопрос на собесе.' }),
  t('be-12', 'backend', 'practice', 'Пагинация, фильтр, сортировка',
    ...
    undefined,
    { realWorld: 'Ни один реальный список не отдают целиком — 100k строк положат и сервер, и браузер. Пагинация + фильтр + сортировка есть в каждом гриде каждого приложения. Skip/Take в SQL — стандарт, который спросят на собесе.' }),
  t('be-13', 'backend', 'practice', 'Кэширование с IMemoryCache',
    ...
    undefined,
    { realWorld: 'Кэш — первый инструмент, когда «надо ускорить»: справочники, редко меняющиеся данные. Но «есть две сложные вещи в IT: инвалидация кэша и...» — устаревший кэш даёт коварные баги. Понимание trade-off ценят на собесе.' }),
  t('be-14', 'backend', 'practice', 'BackgroundService: фоновая задача',
    ...
    { realWorld: 'Фоновые задачи — рассылки, ночная чистка, обработка очереди, периодические отчёты. Работают без участия пользователя. Грабли со scope внутри singleton (тот же DbContext) — классический баг, который отдельно любят на собесе.' }),
  t('be-15', 'backend', 'project', 'Разбери Telegram Mini App слой за слоем',
    ...
    undefined,
    { realWorld: 'Второй проект в портфолио + вторая история для собеса. Интеграция с внешним API (Telegram) — ровно то, чем занимается бэкендер каждый день. Умение рассказать оба проекта уверенно закрывает твой майлстоун готовности.' }),
  t('be-16', 'backend', 'project', 'Новая фича в Auslagenerstattung с нуля',
    ...
    undefined,
    { realWorld: 'Ровно то, что ты будешь делать на работе в первый месяц: взять задачу и провести её через все слои. Сделал сам, без AI — значит готов к джуниор-позиции не на словах. Лучшее доказательство самому себе, что backend-трек усвоен.' }),
];
...
const sql: TaskSeed[] = [
  t('sql-w1', 'sql', 'practice', 'SQL-разминка на SQLBolt',
    ...
    { realWorld: 'SQL — второй язык бэкендера после C#. Даже с EF Core регулярно надо посмотреть данные напрямую, написать отчёт, разобраться, почему запрос медленный. SELECT/WHERE/JOIN — это чтение данных, без которого никуда. И большой блок письменного AP2.' }),
  t('sql-01', 'sql', 'practice', 'JOIN-тренировка',
    ...
    { realWorld: 'JOIN — как связывают таблицы: заказы с клиентами, расходы с категориями. Без него данные бесполезны, они всегда разложены по таблицам. GROUP BY + HAVING — это отчёты и аналитика («сумма по каждому клиенту»). Абсолютный минимум для любой работы с БД и типовое задание AP2.' }),
  t('sql-02', 'sql', 'practice', 'Подзапросы и оконные функции',
    ...
    { realWorld: 'Оконные функции решают «топ-3 в каждой группе», «нарастающий итог», «место в рейтинге» — то, что через обычный GROUP BY либо невозможно, либо уродливо. Сильный сигнал на собесе: junior, который знает window functions, выделяется.' }),
  t('sql-03', 'sql', 'practice', 'План запроса и индекс',
    ...
    { realWorld: 'Индекс — первое, что делают, когда «запрос тормозит». Без него БД читает всю таблицу построчно; с ним — находит сразу. Разница на миллионе строк: секунды против миллисекунд. Реальный навык, за который платят: «страница медленная» → посмотрел план → добавил индекс.' }),
  t('sql-04', 'sql', 'practice', 'Нормализация 1NF–3NF',
    ...
    { realWorld: 'Так проектируют любую БД с нуля: разложить данные по таблицам, чтобы не хранить одно и то же в трёх местах (иначе рассинхрон и баги). Ты делаешь это каждый раз, создавая модели EF Core. И это прямая тема письменного AP2.' }),
  t('sql-05', 'sql', 'practice', 'Какой SQL генерит EF Core',
    ...
    { realWorld: 'EF Core скрывает SQL, но он никуда не делся. Умение посмотреть сгенерированный запрос — как ловят N+1 и тормоза: «почему тут 50 запросов вместо одного?». Это мост между твоим LINQ и реальной нагрузкой на БД.' }),
  t('sql-06', 'sql', 'practice', 'Транзакции и ACID на живом примере',
    ...
    undefined,
    { realWorld: 'Транзакции — «всё или ничего»: списать деньги и зачислить должны произойти вместе, иначе теряются деньги или задваиваются записи. Любая операция, меняющая несколько таблиц (заказ + позиции + склад), просит транзакцию. ACID — обязательный вопрос собеса.' }),
  t('sql-07', 'sql', 'practice', 'CTE: читабельные сложные запросы',
    ...
    undefined,
    { realWorld: 'CTE (WITH ...) превращает нечитаемый вложенный запрос в понятные именованные шаги — коллеги на code review скажут спасибо. Реальный SQL быстро становится сложным, и умение писать его читаемо ценится не меньше, чем умение писать вообще.' }),
  t('sql-08', 'sql', 'practice', 'SQL-марафон: 20 задач',
    ...
    [l('SQL Practice', 'https://www.sql-practice.com/')],
    { realWorld: 'SQL-задачи — частая часть технических собеседований (иногда отдельный этап). И это прямая тренировка письменного AP2, где запросы пишут на бумаге. Объём решённых задач напрямую превращается в скорость и уверенность.' }),
];
...
const tests: TaskSeed[] = [
  t('ts-01', 'tests', 'practice', 'Первые xUnit-тесты (AAA)',
    ...
    { realWorld: 'В любой команде код без тестов не мержат — тесты ловят баги до прода и позволяют менять код без страха что-то сломать. Для джуна тесты в проекте — прямой сигнал «этот человек умеет работать как в команде». У большинства кандидатов их нет.' }),
  t('ts-02', 'tests', 'practice', 'Moq: изоляция зависимостей',
    ...
    { realWorld: 'Мокинг — как тестируют сервис, не трогая настоящую БД или чужой API: подсовываешь фейковый IRepository и проверяешь только свою логику. Без этого юнит-тесты медленные и хрупкие. Стандартный инструмент в каждом .NET-проекте с тестами.' }),
  t('ts-03', 'tests', 'practice', 'Integration-тест через WebApplicationFactory',
    ...
    { realWorld: 'Integration-тест проверяет весь путь (HTTP → контроллер → сервис → БД) — ловит то, что юнит-тесты пропускают: неверный маршрут, кривой маппинг, ошибку конфигурации. Именно такие тесты дают уверенность, что API реально работает. Ценятся выше юнит-тестов.' }),
  t('ts-04', 'tests', 'practice', 'TDD на одном сервисе',
    ...
    { realWorld: 'TDD (сначала тест, потом код) — рабочая практика во многих командах; на собесе спрашивают, знаком ли ты. Побочный эффект: код, написанный «под тест», получается чище и с меньшей связанностью. Полезно хотя бы раз прожить цикл руками.' }),
  t('ts-05', 'tests', 'exam', 'Testverfahren для AP2: Äquivalenzklassen',
    ...
    undefined,
    { realWorld: 'Прямая тема письменного AP2. И реально полезно: «какие случаи протестировать» — это и есть классы эквивалентности и границы (0, 1, max, max+1). Так думают, покрывая код тестами, чтобы не писать 100 бессмысленных проверок.' }),
  t('ts-06', 'tests', 'practice', 'Coverage: найди непокрытое',
    ...
    undefined,
    { realWorld: 'Coverage-отчёт показывает, какие ветки кода тесты вообще не трогали — часто именно там прячутся баги. В командах его гоняют в CI. Но зрелость — понимать, что 100% ради цифры бессмысленно; покрывать надо важное, а не геттеры.' }),
];
...
const devops: TaskSeed[] = [
  t('dv-01', 'devops', 'practice', 'Git: ветки, PR, rebase',
    ...
    { realWorld: 'Git — как работает вся командная разработка: ветка под фичу, Pull Request, ревью, merge. Это не «дополнительно», а ежедневный процесс с первого дня работы. Уверенность в ветках/PR/rebase и разрешении конфликтов ждут по умолчанию.' }),
  t('dv-02', 'devops', 'practice', 'Докеризуй свой API',
    ...
    { realWorld: 'Docker решает «у меня работает, а на сервере нет»: приложение едет вместе со своим окружением. Почти каждая .NET-вакансия сейчас просит Docker. Для тебя это ещё и путь к деплою в Azure (проект P5) — тому самому живому URL в CV.' }),
  t('dv-03', 'devops', 'practice', 'docker-compose: API + БД одной командой',
    ...
    { realWorld: 'docker-compose поднимает всё окружение (API + БД + кэш) одной командой — так новый разработчик заводит проект за 5 минут вместо дня настройки. Твой compose-файл в README = «этот человек умеет упаковать проект». Часть проекта P5.' }),
  t('dv-04', 'devops', 'practice', 'Деплой контейнера в Azure',
    ...
    { realWorld: 'Это и есть тот самый пункт, который сильнее всего выделит тебя среди джунов: живая ссылка в CV, которую рекрутер может открыть и потыкать. «Задеплоил своё в Azure» на собеседовании звучит совсем иначе, чем «писал функции в IDE». Твоя цель №2 буквально про это.' }),
  t('dv-05', 'devops', 'practice', 'CI/CD: GitHub Actions',
    ...
    { realWorld: 'CI/CD — автоматика, которая на каждый push сама собирает, гоняет тесты и деплоит: сломанный код не доедет до прода. Это стандарт индустрии и сильный пункт в CV джуна. Зелёный badge в README сразу говорит «я так умею».' }),
  t('dv-06', 'devops', 'theory', 'AZ-900 спринт (опционально)',
    ...
    { realWorld: 'AZ-900 — строчка в CV, по которой HR фильтрует, плюс общий язык об облаке (что такое регион, ресурсная группа, PaaS vs IaaS). Для .NET-вакансий с Azure это ощутимый плюс. Знания нужны в любом случае — сертификат опционален.' }),
  t('dv-07', 'devops', 'practice', 'Секреты и окружения по-взрослому',
    ...
    { realWorld: 'Ключ или пароль, случайно закоммиченный в git, — реальная дыра: боты сканируют публичные репо и находят их за минуты. Правильная работа с секретами (user secrets / GitHub Secrets / Key Vault) — базовая гигиена, которую проверяют и в проде, и на собесе по безопасности.' }),
  t('dv-08', 'devops', 'practice', 'Health checks и мониторинг',
    ...
    { realWorld: '/health — как облако и балансировщик понимают, жив ли сервис (и надо ли перезапустить). Application Insights показывает реальные запросы, ошибки и медленные места на проде. «Как узнать, что с приложением после деплоя» — это про эксплуатацию, что сильно отличает тебя от джуна, умеющего только писать код.' }),
  t('dv-09', 'devops', 'project', 'Задеплой второй проект',
    ...
    undefined,
    { realWorld: 'Второй деплой превращает «однажды получилось по гайду» в навык, который ты повторишь на работе с любым проектом. Плюс вторая живая ссылка в портфолио — оба проекта можно открыть и потыкать прямо на собеседовании.' }),
];
...
const exam: TaskSeed[] = [
  t('ex-01', 'exam', 'exam', 'Диагностика: старая AP2 целиком по времени',
    ...
    { realWorld: 'Репетиция самого дня пересдачи 25.11: то же время, тот же формат. Диагностика показывает твой реальный старт и точки провала — без неё готовишься вслепую. Одна из важнейших задач всей программы.' }),
  t('ex-02', 'exam', 'exam', 'Pseudocode: базовый тренажёр',
    ...
    { realWorld: 'Прямые баллы письменного AP2 (твой слабейший блок Algorithmen, 24 балла). Псевдокод пишут руками на бумаге — набитая рука экономит время и нервы на экзамене.' }),
  t('ex-03', 'exam', 'exam', 'Struktogramme: читать и рисовать',
    ...
    [l('Structorizer (редактор)', 'https://structorizer.fisch.lu/')],
    { realWorld: 'Типовое задание AP2: дают Struktogramm — напиши код, или наоборот. Баллы идут за скорость и точность в обозначениях; на экзамене некогда вспоминать, как рисуется цикл с постусловием.' }),
  t('ex-04', 'exam', 'exam', 'Трассировка алгоритмов на бумаге',
    ...
    undefined,
    { realWorld: 'Классический формат AP2: «что выведет этот код» — заполняешь таблицу значений по шагам. Легко потерять баллы на невнимательности, поэтому механику тренируют на бумаге. Плюс это навык отладки кода в голове.' }),
  t('ex-05', 'exam', 'exam', 'ER-модель и UML по описанию',
    ...
    { realWorld: 'Прямая тема письменного AP2: по описанию нарисовать ER-модель и классовую диаграмму. И то же самое ты делаешь, проектируя БД и классы в реальном проекте — экзамен и работа тут совпадают.' }),
  t('ex-06', 'exam', 'exam', 'Нормализация + SQL на бумаге',
    ...
    undefined,
    { realWorld: 'Баллы AP2 за нормализацию и SQL-запросы, написанные РУКАМИ — без автодополнения IDE ошибки другие. Тот же навык (спроектировать схему, написать JOIN) — ежедневная работа с БД.' }),
  t('ex-07', 'exam', 'exam', 'IT-gestützter Arbeitsplatz: типовой блок',
    ...
    undefined,
    { realWorld: 'Отдельный блок письменного AP2: эргономика, Datenschutz, лицензии, подбор железа. Дешёвые баллы, если знаешь типовые вопросы — грех терять на том, что просто выучить.' }),
  t('ex-08', 'exam', 'exam', 'WISO блок 1: трудовое право',
    ...
    { realWorld: 'Прямые баллы блока WISO (47 баллов на твоей пересдаче). Плюс это буквально твоя жизнь: Arbeitsvertrag, права, Betriebsrat пригодятся, когда будешь подписывать первый рабочий договор.' }),
  t('ex-09', 'exam', 'exam', 'WISO блок 2: соцстрахование и договоры',
    ...
    [l('Prozubi: WISO', 'https://www.prozubi.de/')],
    { realWorld: 'Баллы WISO — и практическая грамотность: из чего складывается вычет из зарплаты (5 столпов), какие сроки Kündigung. Понадобится буквально при устройстве на работу (цель №2).' }),
  t('ex-10', 'exam', 'exam', 'Старая Prüfung №2 по времени',
    ...
    [l('U-Form: старые Prüfungen', 'https://www.u-form-shop.de/')],
    { realWorld: 'Второй полный прогон по времени — замер прогресса относительно диагностики. На экзамене решает не только знание, но и умение уложиться в 90 минут; это тренируется только полными прогонами.' }),
  t('ex-11', 'exam', 'exam', 'Разбор ошибок №2 + добивка слабых тем',
    ...
    undefined,
    { realWorld: 'Разбор ошибок — где баллы растут быстрее всего: отделить «не знал» от «невнимательность» и закрыть каждую. Именно это превращает прогон в реальный прирост к экзамену.' }),
  t('ex-12', 'exam', 'exam', 'Prüfung №3: генеральная репетиция',
    ...
    undefined,
    { realWorld: 'Генеральная репетиция в условиях экзамена — чтобы 25.11 формат был уже привычным, а не стрессом. После неё останется точечный список тем на последнюю неделю.' }),
  t('ex-14', 'exam', 'exam', 'Netzplan и Gantt: проектное планирование',
    ...
    undefined,
    { realWorld: 'Типовое задание письменной части: критический путь, буферы, сроки проекта. И реальный инструмент планирования — так прикидывают, к какому сроку успеет фича и что нельзя задержать.' }),
  t('ex-15', 'exam', 'exam', 'Lastenheft, Pflichtenheft, Qualität',
    ...
    undefined,
    { realWorld: 'Баллы AP2 за терминологию проекта и качества ПО. В работе это язык, на котором обсуждают требования: что хочет заказчик (Lastenheft) и как мы это сделаем (Pflichtenheft).' }),
  t('ex-16', 'exam', 'exam', 'OOP на бумаге: диаграмма → код',
    ...
    undefined,
    { realWorld: 'Прямая тема AP2 (наследование, полиморфизм по диаграмме). Это же ядро повседневного C#: спроектировать классы и связи. Экзамен проверяет ровно тот навык, что нужен на работе.' }),
  t('ex-17', 'exam', 'exam', 'WISO блок 3: договоры и право',
    ...
    [l('Prozubi: WISO', 'https://www.prozubi.de/')],
    { realWorld: 'Баллы WISO — и бытовая грамотность: Kaufvertrag, гарантия (Gewährleistung), права потребителя. Пригодится в жизни не меньше, чем на экзамене.' }),
  t('ex-18', 'exam', 'exam', 'WISO блок 4: экономика',
    ...
    [l('Prozubi: WISO', 'https://www.prozubi.de/')],
    { realWorld: 'Регулярные темы блока WISO: инфляция, EZB, экономические циклы. Формулы и определения — чистые баллы, если выучены; на экзамене их или знаешь, или нет.' }),
  t('ex-19', 'exam', 'exam', 'Стратегия экзамена: время и порядок',
    ...
    undefined,
    { realWorld: 'Тактика дня X: как распределить 90 и 60 минут, что решать первым, когда пропускать. На пересдаче время — главный ограничитель; заранее продуманный план экономит несколько баллов.' }),
  t('ex-13', 'exam', 'exam', 'Финальный прогон WISO и формул',
    ...
    undefined,
    { realWorld: 'Последняя проверка перед 25.11: прогнать все карточки и смешанный блок, найти оставшиеся дыры. То, что повторишь накануне, с наибольшей вероятностью встретишь на экзамене.' }),
];
...
const job: TaskSeed[] = [
  t('job-01', 'job', 'practice', 'CV на немецком (1 страница)',
    ...
    undefined,
    { realWorld: 'Твоё CV — первое (а часто и единственное за 10 секунд), что видит работодатель. Один экран, стек + живой проект наверху. От него напрямую зависит, позовут ли на собеседование. Прямо ведёт к цели №2.' }),
  t('job-02', 'job', 'practice', 'README для каждого проекта',
    ...
    undefined,
    { realWorld: 'Рекрутер и техлид открывают твой GitHub и первым делом читают README — это витрина проекта. Хороший README (что, зачем, как запустить, живой URL) часто решает больше, чем сам код внутри.' }),
  t('job-03', 'job', 'practice', 'LinkedIn + Xing: Open to Work',
    ...
    undefined,
    { realWorld: 'В Германии рекрутеры активно ищут в Xing и LinkedIn. «Open to work» + заполненный профиль = входящие приглашения, а не только твои исходящие заявки. Целый дополнительный канал воронки трудоустройства.' }),
  t('job-04', 'job', 'practice', 'Рассказ о проектах за 2 минуты',
    ...
    undefined,
    { realWorld: 'Почти каждое собеседование начинается с «расскажите о вашем проекте». Гладкий рассказ за 2 минуты (проблема → решение → стек) задаёт тон всей встрече. Репетируется заранее, иначе плывёшь на самом старте.' }),
  t('job-05', 'job', 'theory', 'Честный нарратив про AI-assisted проекты',
    ...
    undefined,
    { realWorld: 'Тебя точно спросят, как ты делал проекты. Честная и уверенная позиция («начал с AI, переписал, понимаю каждый слой») — сила, а не слабость. Заготовленный ответ снимает главный твой страх на собесе.' }),
  t('job-06', 'job', 'practice', 'Первые 5 заявок',
    ...
    { realWorld: 'Собственно трудоустройство и начинается с отправки заявок — это цель №2 в действии, а не подготовка к ней. Первые пять учат адаптировать CV под конкретную вакансию. Трекер тут же покажет твою воронку.' }),
  t('job-07', 'job', 'practice', '20 вопросов с собесов → карточки',
    ...
    { realWorld: 'Вопросы .NET-собеса повторяются от компании к компании. Собрал 20, ответы — в карточки, повторяешь — и на реальном собесе они уже отрепетированы, а не «где-то слышал». Прямая подготовка к цели №2.' }),
  t('job-08', 'job', 'practice', 'Mock-интервью с AI',
    ...
    undefined,
    { realWorld: 'Безопасная репетиция перед реальным собеседованием: провалиться с AI не страшно, провалиться на настоящем — упущенный оффер. Вскрывает то, что ты вроде знаешь, но не можешь внятно сказать вслух.' }),
  t('job-09', 'job', 'practice', 'Добей до 20+ заявок',
    ...
    undefined,
    { realWorld: 'Трудоустройство — игра чисел: больше качественных заявок → больше собеседований → выше шанс оффера. 20+ заявок с анализом откликов — это и есть активный поиск, а не пассивное ожидание.' }),
  t('job-10', 'job', 'practice', 'Anschreiben-шаблон',
    ...
    undefined,
    { realWorld: 'Немецкие вакансии часто ждут Anschreiben (сопроводительное письмо). Готовый шаблон, адаптируемый за 10 минут, снимает барьер «лень писать письмо» и напрямую повышает число отправленных заявок.' }),
  t('job-11', 'job', 'practice', 'Нетворкинг: 5 контактов',
    ...
    undefined,
    { realWorld: 'Часть вакансий закрывается через знакомства и рекомендации, а не через порталы. 5 сообщений в неделю + пара сообществ = скрытый канал трудоустройства, которым пользуются немногие джуны.' }),
  t('job-12', 'job', 'practice', 'Изучи зарплатные вилки',
    ...
    undefined,
    { realWorld: 'Вопрос о зарплате на собесе застаёт врасплох и стоит реальных денег. Знаешь вилку Junior .NET в NRW — называешь адекватный диапазон уверенно, не продешевишь и не отпугнёшь работодателя.' }),
  t('job-13', 'job', 'practice', 'Заявки 21–35 + анализ воронки',
    ...
    undefined,
    { realWorld: 'Анализ воронки показывает, где ты теряешь: мало ответов — проблема в CV; мало офферов после интервью — в подготовке к собесу. Данные вместо гадания и слепой рассылки.' }),
  t('job-14', 'job', 'practice', 'Разбор реального интервью',
    ...
    undefined,
    { realWorld: 'Каждое реальное собеседование — бесплатный список твоих слабых мест. Выписал вопросы, где поплыл, в карточки — и к следующему собесу их закрыл. Так растут быстрее всего.' }),
  t('job-15', 'job', 'practice', 'Второе mock-интервью: замер прогресса',
    ...
    undefined,
    { realWorld: 'Замер прогресса через месяц: сравниваешь с первым mock и видишь рост чёрным по белому. Мотивирует и показывает, что ещё осталось добить перед решающими собеседованиями.' }),
];
